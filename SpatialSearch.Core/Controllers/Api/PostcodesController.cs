...
namespace SpatialSearch.Core.Controllers.Api
{
    ...
    public class PostcodesController : UmbracoApiController
    {
        ...
        public string AddRandomPostcodeToDatabase(AddRandomPostcodesToDatabaseModel model)
        {
            if (!ModelState.IsValid)
            {
                throw new BadHttpRequestException("Postcode is invalid", (int)HttpStatusCode.BadRequest);
            ...
            for (int i = 0; i < model.Number; i++)
            {
                try
                {
                    var locationData = _postcodeApiService.GetRandomPostcodeData().Result;
                    _dataHandlerService.WritePostcodeToDatabase(locationData);
                }
                catch
                {
                    continue;
            ...
            return "Postcodes Added Successfully";
        }
    }
}
